  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/sync") {
      try {
        ...
        // الدوران حول الوظائف وحفظها في قاعدة البيانات D1
        ...
        return new Response(JSON.stringify({ success: true, message: `Synced ${jobs.length} jobs successfully!` }), {
          headers: { "Content-Type": "application/json;charset=UTF-8" }
        });
      } catch (error) {
...
<body>
    ...
    <script>
        ...
        btn.addEventListener('click', async () => {
            ...
            statusDiv.innerText = 'Mining and syncing jobs...';
            try {
